...
@pytest.fixture
def tightened_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    """The §9.5 M6 step 5 contract: both retired columns are gone from it.

    The gate reads the classification the *contract* declares, so this tightens
    the one dict every importer shares rather than a name in the migration
    module — which is exactly what the release does when the literal changes.
    ``monkeypatch.delitem`` puts the keys back afterwards.
    """

    assert module.POSITION_LOTS_COLUMN_CLASSIFICATION is (
        repository_common.POSITION_LOTS_COLUMN_CLASSIFICATION
    ), "the gate must read the contract's own dict, not a copy of it"
    for name in module.RETIRED_LOT_COLUMNS:
        monkeypatch.delitem(
            repository_common.POSITION_LOTS_COLUMN_CLASSIFICATION, name
        )
    assert module._contract_still_declares_retired_columns() == ()
...
# --- the derived contract gate ------------------------------------------------
#
# D1/D2's rebuild and D3/D4's payload rewrite are destructive, and §13.5 R7 fixes
# what makes them safe: the column contract is exact-set equality *and* the
# publish path's hard door, so a rebuilt table is ``column_contract_open`` →
# head ``untrusted`` → tail publish ``RuntimeError`` for as long as the running
# code still declares the retired columns. The gate below is therefore read from
# the contract in force, never from a flag: the release that tightens the
# contract is the release that enables the migration.


def test_the_destructive_steps_are_deferred_while_the_contract_declares_them(
    tmp_path: Path,
) -> None:
    ...
    path = _legacy_store(tmp_path)
    assert set(module.RETIRED_LOT_COLUMNS) <= set(
        module.POSITION_LOTS_COLUMN_CLASSIFICATION
    )
    assert module._contract_still_declares_retired_columns() == (
        "expiration",
        "record_id",
    ...
    for deferred in (
        ...
        assert steps[deferred]["status"] == "deferred"
        assert steps[deferred]["reason"] == "column_contract_precedes_rebuild"
    assert steps["rewrite_fields_json_to_lot_shape"]["status"] == "deferred"
    assert steps["rewrite_fields_json_to_lot_shape"]["reason"] == (
        "read_side_compatibility_window_open"
    )
    # The store keeps the shape the contract still declares: the gate closed the
    # destructive half, not just its report.
    assert "record_id" in _columns(path)
    assert "expiration" in _columns(path)
    assert _primary_key(path) == "record_id"


def test_tightening_the_contract_is_what_enables_the_destructive_steps(
    tmp_path: Path,
    tightened_contract: None,
) -> None:
    """The gate is derived state, so nothing else has to be flipped with it.

    The same store, the same code, the same command: only the contract moved.
    This is the property that keeps M6 step 5 from needing a second edit here —
    and the reason a flag would be the wrong shape for it.
    """
...
def test_the_default_paths_report_is_unchanged_while_the_gate_is_closed(
    tmp_path: Path,
) -> None:
    """The pre-window receipt is a contract this batch must not move.
    ...
    three are ``deferred`` with the same reasons, ``write_applied`` still means
    "something changed", and no rebuild report appears at all.
    """
    ...
    assert result["steps"] == [
        ...
        },
        {
            ...
            "status": "deferred",
            "reason": "column_contract_precedes_rebuild",
        },
        {
            "item": "D1",
        ...
        },
        {
            ...
            "status": "deferred",
            "reason": "read_side_compatibility_window_open",
        },
    ]
    assert result["write_applied"] is True
    ...
    # closed the payload keeps the read-model shape the readers still consume —
    ...
    for row in _stored_rows(path).values():
        ...
        assert row["fields"]["account"]
        assert row["fields"]["symbol"]
        assert "contract_key" not in row["fields"]
    assert _primary_key(path) == "record_id"
...
def test_the_rebuild_leaves_the_store_on_the_contracted_shape(
    ...
    """§13.4 A8: the rebuilt store's column set *is* the tightened contract.

    The expectations come from ``POSITION_LOTS_COLUMN_CLASSIFICATION``, so the
    shape the rebuild produces and the shape the publish path gates on cannot
    drift apart — and a rebuilt store is not ``column_contract_open``.
    """
    ...
    assert set(_columns(path)) == set(module.POSITION_LOTS_COLUMN_CLASSIFICATION)
    assert _primary_key(path) == "lot_id"
    ...
    # lot_id, and everything else is the byte the old row held.
    for lot_id, row in rows.items():
        assert row["source_event_id"] == before[lot_id]["fields"]["source_event_id"]
        assert row["account"] == before[lot_id]["fields"]["account"]
    ...
    with sqlite3.connect(path) as conn:
        ...
        # The contract check the publish path runs, verbatim: a rebuilt store is
        # not ``column_contract_open``, which is the whole reason the gate waits
        # for the contract to tighten before it lets the rebuild run (§13.5 R7).
        ...
        assert _position_projection_column_contract(conn)["position_lots"] == {
            "missing": (),
            "unclassified": (),
        }
        assert _position_projection_column_contract_is_closed(conn) is True

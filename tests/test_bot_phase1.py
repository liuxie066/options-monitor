...
def test_agent_tool_view_exposes_result_contract() -> None:
    view = next(item for item in bot_tools.tool_descriptions(("option_performance_report",)))
    ...
    warning_observation = bot_tools.compact_observation(
        "option_performance_report",
        {
            "ok": True,
            "data": {"rows": []},
    ...
    nested = bot_tools.compact_observation(
        "option_performance_report",
        {
            "ok": True,
            "data": {
...
def test_event_cursor_is_exact_for_model_and_hashed_in_audit() -> None:
    ...
    audit = bot_tools.audit_tool_event_payload(
...
def test_bot_allows_host_owned_hidden_runtime_log_inputs() -> None:
    payload, error = bot_tools.build_tool_payload(
        "runtime_logs",
...
def test_bot_runtime_log_observation_uses_only_allowlisted_metadata() -> None:
    observation = bot_tools.compact_observation(
        "runtime_logs",
        {
            "ok": True,
...
def test_observation_projection_prioritizes_contract_facts_and_missing_boundaries() -> None:
    ...
    observation = bot_tools.compact_observation("option_performance_report", response)
    ...
    partial = bot_tools.compact_observation("option_performance_report", response)
...
def test_prepared_contract_reloads_current_portfolio_toolset_on_resume(monkeypatch, tmp_path) -> None:
    from src.application.bot import local_harness
    ...
    monkeypatch.setattr("src.application.bot.host.run_pi_agent", fake_pi_agent(model))
    monkeypatch.setattr(bot_tools, "call_read_tool", fake_call)
...
def test_host_constrains_option_performance_cutoff_before_business_read(
    ...
    assert tool_event.payload["model_input"] == bot_tools.audit_tool_input(
        "option_performance_report",
        arguments,
        model_proposal=True,
    )
...
def test_host_fixed_month_scope_binds_the_complete_period(monkeypatch, question: str) -> None:
    request = replace(
        _request(question),
        explicit_scope=BotScope(config_key="us", month="2026-08"),
    )
...
def test_budget_exhaustion_forces_final_text_without_tools(monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.application.bot.host.build_scene_manifest", fake_manifest)
    monkeypatch.setattr(
        bot_tools,
        "call_read_tool",
...
def test_host_store_persists_run_events(tmp_path) -> None:
    store = BotHostStore(tmp_path / "bot.db")
    ...
    reopened = BotHostStore(tmp_path / "bot.db")

...
def test_read_view_strips_retired_ai_overlay_from_historical_brief(
    ...
    assert "ai_decision_advice_evidence_index" not in view["brief"]
    assert "AI建议-RETIRED-MUST-NOT-LEAK" not in view["rendered_markdown"]
    assert "RETIRED-EVIDENCE-MUST-NOT-LEAK" not in str(view)

...
def _observation() -> dict:
    ...
    return {
        ...
        "market": "US",
        "contract_identity": {
            "symbol": "NVDA", "option_contract_code": "US.NVDA260821P100000",
            "option_type": "put", "position_side": "short", "strike": "100.00",
        ...
        "query_window": {"start": "2026-08-20", "end": "2026-09-04"},
        "calendar_hash": "calendar-v1",
        "required_sources": [
            "anchor_option_close", "history_deals", "history_orders",
            "fresh_positions", "trading_calendar", "contract_metadata",
        ],
        "source_receipts": {
            "anchor_option_close": _receipt(
                [
                    {
                        "evidence_id": "anchor-1", "source_event_id": anchor_key,
                        "account": "lx", "futu_account_id": "1001",
                        "symbol": "NVDA", "option_type": "put", "position_side": "short",
                        "strike": 100, "expiration_ymd": "2026-08-21", "contracts": 1,
                        "price": 0, "event_time_ms": 1_800_000_000_000,
                        "received_at_ms": 1_800_000_000_100, "order_id": "order-1",
                    }
                ]
            ),
                ...
                [
                    {
                        ...
                        "code": "US.NVDA260821P100000", "qty": "1.0", "price": "0",
                        "order_id": "order-1",
                    },
                    {
                        "deal_id": "unrelated-account-deal", "acc_id": "1001",
                        "code": "US.AAPL", "qty": 100, "price": 200,
                    },
                ]
            ),
            "history_orders": _receipt(
                [
                    {"order_id": "order-1", "is_broker_auto": True, "order_origin": "broker"},
            ...
            "contract_metadata": _receipt(
                [
                    {
                        "settlement_style": "physical", "underlying_security_type": "equity",
                        "last_trade_cutoff_ms": 1_800_000_000_000,
                        "last_trade_cutoff_source": "registry", "calendar_hash": "calendar-v1",
                    }
                ]
            ),
        },
...
def _fingerprint(observation: dict) -> str:
    return str(attach_settlement_semantics(observation)["semantic_fingerprint"])
...
def test_current_semantic_envelope_round_trips_canonical_projection() -> None:
    ...
    evidence["semantic_projection"] = dict(reversed(list(evidence["semantic_projection"].items())))
...
def test_current_semantic_projection_mismatch_fails_closed(
    ...
    target["semantic_projection"] = deepcopy(target["semantic_projection"])
    target["semantic_projection"]["complete"] = False

    with pytest.raises(
        SettlementSemanticUnavailable, match=f"projection mismatch in {location}"
    ):
...
def test_partial_current_semantic_metadata_fails_closed() -> None:
    ...
    with pytest.raises(
        SettlementSemanticUnavailable, match="projection is unavailable in evidence"
    ):
        settlement_semantic_from_evidence(evidence)
...
def test_row_order_unknown_columns_and_unrelated_rows_are_ignored() -> None:
    ...
    deals.append({"deal_id": "another-unrelated-deal", "acc_id": "1001", "code": "US.TSLA", "qty": 1})
...
def test_position_fallback_filters_rows_by_case_contract() -> None:
    ...
    baseline["incomplete_reason_codes"] = ["option_contract_code_missing"]

    unrelated = deepcopy(baseline)
    unrelated["source_receipts"]["fresh_positions"]["rows"].append(
        {
            "code": "US.AAPL260821P200000", "qty": 1, "option_type": "put",
            "expiration_ymd": "2026-08-21", "strike": "200.00",
        }
    )
    matching = deepcopy(baseline)
    matching["source_receipts"]["fresh_positions"]["rows"].append(
        {
            "code": "US.NVDA260821P100000", "qty": 1, "option_type": "PUT",
            "expiration": "2026-08-21", "strike": 100,
        }
    )
...
def test_relevant_duplicate_row_multiplicity_changes_semantics() -> None:
    first = _observation()
    second = deepcopy(first)
    relevant = deepcopy(second["source_receipts"]["history_deals"]["rows"][0])
    second["source_receipts"]["history_deals"]["rows"].append(relevant)
...
def test_query_coverage_and_business_facts_change_semantics() -> None:
    ...
    changed_window["source_receipts"]["history_deals"]["query_input"]["end"] = "2026-09-05"
    changed_coverage = deepcopy(baseline)
    changed_coverage["source_receipts"]["history_deals"]["coverage_complete"] = False

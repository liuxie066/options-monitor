...
from tests.bot_pi_test_support import ModelRequest, ModelTurn, ToolCall, run_contract
...
def test_freeform_answer_quality_scenarios(monkeypatch, scenario: Scenario) -> None:
    ...
    monkeypatch.setattr(bot_tools, "call_read_tool", call_tool)
...
def test_freeform_write_request_cannot_execute(monkeypatch) -> None:
    ...
    def model(request: ModelRequest) -> ModelTurn:
        ...
        return ModelTurn(text="这个 Bot 是只读环境，不能修改配置；请使用显式配置命令并完成确认。")

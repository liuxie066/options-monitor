...
def _patch_common(mod):
    ...
    mod.sh = lambda _cmd, _cwd, capture=True: SimpleNamespace(returncode=0)  # type: ignore[assignment]
...
def test_send_if_needed_validates_delivery_plan_even_when_not_sending() -> None:
    ...
    try:
        ...
        mod.DeliveryPlan.from_payload = classmethod(  # type: ignore[method-assign]
            lambda _cls, _raw: (_ for _ in ()).throw(mod.SchemaValidationError("bad delivery"))
        )
        sys.argv = [
            "send_if_needed.py",
            "--config",

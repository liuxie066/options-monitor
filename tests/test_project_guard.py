...
def test_project_guard_rate_limit(tmp_path: Path) -> None:
    td = tmp_path
    root = Path(td)
...
def test_project_guard_circuit_and_half_open_probe_load_shed(tmp_path: Path) -> None:
    td = tmp_path
    root = Path(td)
...
def test_project_guard_success_closes_circuit(tmp_path: Path) -> None:
    td = tmp_path
    root = Path(td)

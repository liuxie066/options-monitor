...
def test_scheduled_validation_is_cached(tmp_path: Path) -> None:
    ...
    load_config(base=tmp_path, config_path=cfg_path, is_scheduled=True, log=_noop_log, validate_config_fn=_validate, state_dir=state_dir)
    load_config(base=tmp_path, config_path=cfg_path, is_scheduled=True, log=_noop_log, validate_config_fn=_validate, state_dir=state_dir)
...
def test_scheduled_validation_failure_is_not_cached(tmp_path: Path) -> None:
    from src.application.config_loader import load_config
...
def test_scheduled_validation_rechecks_same_hash_when_validator_version_changes(tmp_path: Path) -> None:
...
def test_resolve_data_config_path_prefers_explicit_path(tmp_path: Path) -> None:
    from src.application.config_loader import resolve_data_config_path

    explicit = tmp_path / "custom.json"
    explicit.write_text("{}", encoding="utf-8")

    out = resolve_data_config_path(base=tmp_path, data_config="custom.json")

    assert out == explicit.resolve()


def test_default_data_config_path_prefers_runtime_config_location_when_present(tmp_path: Path) -> None:
    ...
    out = default_data_config_path(base=tmp_path)

    assert out == data_config.resolve()


def test_default_data_config_path_falls_back_to_runtime_config_location_when_missing(tmp_path: Path) -> None:
    from src.application.config_loader import default_data_config_path

    out = default_data_config_path(base=tmp_path)

    assert out == (tmp_path / "portfolio.runtime.json").resolve()


def test_data_config_candidates_use_runtime_location_only(tmp_path: Path) -> None:
    from src.application.config_loader import data_config_candidates

    out = data_config_candidates(base=tmp_path)

    assert out == [(tmp_path / "portfolio.runtime.json").resolve()]


def test_resolve_data_config_path_prefers_env_override(monkeypatch, tmp_path: Path) -> None:
    from src.application.config_loader import resolve_data_config_path

    env_path = tmp_path / "external" / "portfolio.feishu.json"
    monkeypatch.setenv("OM_DATA_CONFIG", str(env_path))

    out = resolve_data_config_path(base=tmp_path, data_config=None)

    assert out == env_path.resolve()


def test_resolve_data_config_path_ignores_legacy_om_pm_config(monkeypatch, tmp_path: Path) -> None:
    from src.application.config_loader import resolve_data_config_path

    legacy_env_path = tmp_path / "external" / "legacy-portfolio.json"

...
def test_assistant_memory_proposal_lifecycle_commands(tmp_path: Path, capsys) -> None:
    import src.interfaces.cli.main as cli
    ...
    assert rc == 0
    assert payload["tool_name"] == "assistant.memory.propose"
    assert payload["data"]["write_applied"] is True
    assert payload["data"]["proposal"]["status"] == "proposed"
    ...
    assert rc == 0
    assert payload["tool_name"] == "assistant.memory.list_proposals"
    assert payload["data"]["proposal_count"] == 1
    assert payload["data"]["proposals"][0]["proposal_id"] == "proposal_parameter_tuning"
    ...
    assert rc == 0
    assert payload["tool_name"] == "assistant.memory.accept"
    assert payload["data"]["proposal"]["status"] == "accepted"
    assert (memory_dir / "parameter-tuning.md").exists()


def test_assistant_memory_reject_command(tmp_path: Path, capsys) -> None:
    import src.interfaces.cli.main as cli
    ...
    rc = cli.main([
        ...
        "工作流偏好",
        "--summary",
        "用户偏好先读 authority path。",
        "--content",
        "诊断前先确认当前 authority path。",
    ])
    ...
    assert rc == 0
    assert "Rejected assistant memory proposal: proposal_workflow" in text
    assert not (memory_dir / "workflow-pattern.md").exists()


def test_assistant_memory_suggest_command_creates_proposal_only(tmp_path: Path, capsys) -> None:
    import src.interfaces.cli.main as cli
    ...
    rc = cli.main([
        ...
        "turn_789",
        "--text",
        "请记住：调参时先看 replay 和拒绝原因。",
    ])
    payload = _read_json_output(capsys)

    assert rc == 0
    assert payload["tool_name"] == "assistant.memory.suggest"
    data = payload["data"]
    assert data["write_applied"] is True
    assert data["proposal_count"] == 1
    assert data["proposals"][0]["type"] == "parameter_tuning_preference"
    assert data["proposals"][0]["source_turn"] == "turn_789"
    ...
    assert rc == 0
    assert "No safe assistant memory suggestions found." in text

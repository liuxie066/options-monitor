...
from src.application.bot.contracts import AppEvent, AppResult
from scripts import bot_p1_eval
...
def _response(question: str) -> str:
    case = next(item for item in bot_p1_eval.CASES if item.question == question)
...
def test_p1_eval_readability_guards_enforce_order_and_hide_evidence_ids() -> None:
    assert bot_p1_eval._terms_in_order(
        "期权净现金流 USD 300；卖出期权胜率 75%；期权收益率 12%。",
        ("期权净现金流", "卖出期权胜率", "期权收益率"),
    )
    assert not bot_p1_eval._terms_in_order(
        "期权收益率 12%；卖出期权胜率 75%；期权净现金流 USD 300。",
        ("期权净现金流", "卖出期权胜率", "期权收益率"),
    )
    assert bot_p1_eval._contains_raw_evidence_identifier(
        "缺失证据 source_event_id=event-private-001"
    )
    assert not bot_p1_eval._contains_raw_evidence_identifier(
        "费用证据不完整，因此仅报告毛额。"
    )
...
def test_p1_eval_rejects_incomplete_human_review(tmp_path) -> None:
    ...
    with pytest.raises(SystemExit) as _caught:
        bot_p1_eval._load_human_reviews(str(path))
    exc = _caught.value
...
def test_apply_human_reviews_updates_existing_report_without_running_model() -> None:
    report = {
        "schema_version": "om.bot.p1_eval.v4",
        "structural_pass": True,
        "answer_quality_pass": None,
        ...
        "cases": [
            {
                "name": "july_income",
                "response": "结论：原始答案",
                "human_review": bot_p1_eval._empty_human_review(),
                "human_score": None,
                "answer_quality_pass": None,
            }
        ],
    ...
    reviewed = bot_p1_eval.apply_human_reviews(report, {"july_income": scores})
...
def test_apply_human_reviews_requires_exact_case_set() -> None:
    report = {"schema_version": "om.bot.p1_eval.v4", "cases": [{"name": "july_income"}]}
    scores = {dimension: 2 for dimension in bot_p1_eval._empty_human_review()}

    with pytest.raises(SystemExit) as _caught:
        bot_p1_eval.apply_human_reviews(report, {"other": scores})
    exc = _caught.value
    assert "missing=['july_income']" in str(exc)
    assert "unknown=['other']" in str(exc)


@pytest.mark.parametrize("schema", [None, "om.copilot.p1_eval.v4", "om.bot.p1_eval.v3"])
def test_review_report_rejects_non_bot_schema_before_output(tmp_path, monkeypatch, schema) -> None:
    report = {"schema_version": schema, "cases": [{"name": "case"}],
              "structural_pass": True, "evidence_pass": True}
    report_path = tmp_path / "report.json"
    report_text = json.dumps(report)
    report_path.write_text(report_text)
    scores_path = tmp_path / "scores.json"
    scores_path.write_text(json.dumps({"case": {key: 2 for key in bot_p1_eval._empty_human_review()}}))
    output = tmp_path / "reviewed.json"
    monkeypatch.setattr(sys, "argv", ["bot_p1_eval.py", "--review-report", str(report_path),
                                      "--review-input", str(scores_path), "--output", str(output)])
    def no_model(**kwargs):
        pytest.fail("reviewing a saved report must never run a model")
    monkeypatch.setattr(bot_p1_eval, "run_eval", no_model)
    with pytest.raises(SystemExit, match="schema_version must be om.bot.p1_eval.v4"):
        bot_p1_eval.main()

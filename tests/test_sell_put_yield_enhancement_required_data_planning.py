...
def test_diagonal_combo_yield_preserves_explicit_call_dte_window(monkeypatch, tmp_path: Path) -> None:
    import src.application.required_data_planning as mod
    ...
    put_plan = next(side for side in plan.side_plans if side.option_type == "put")
    call_plan = next(side for side in plan.side_plans if side.option_type == "call")
    assert put_plan.min_dte == 20
    assert put_plan.max_dte == 60
    assert call_plan.min_dte == 61
    assert call_plan.max_dte == 90

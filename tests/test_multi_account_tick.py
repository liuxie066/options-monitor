...
def test_ai_observation_publish_unions_four_sources_and_preserves_other_market(
    tmp_path: Path,
) -> None:
    from src.application import multi_account_tick as mod
    ...
    status = mod._publish_ai_decision_observation_partitions(
        base=tmp_path,
        run_id="us-new",
        markets=("US",),
        config={"symbols": [{"symbol": "AAPL"}]},
        successful_accounts=("lx", "sy"),
        candidate_snapshots_by_account={
            "lx": candidate_snapshot,
            "sy": {"ranked_candidates": []},
        },
        portfolio_distributions_by_account={"sy": portfolio},
        option_contexts_by_account={"lx": option_context},
    )

    assert status == {"US": {"status": "ready", "symbol_count": 4}}
    snapshot = load_observation_set(tmp_path)
    assert snapshot is not None
    assert snapshot["partitions"]["HK"]["generation"] == "hk-old"
    assert snapshot["partitions"]["HK"]["symbols"] == [
        {"symbol": "0700.HK", "market": "HK", "priority": 4}
    ]
    assert snapshot["partitions"]["US"]["generation"] == "us-new"
    assert snapshot["partitions"]["US"]["symbols"] == [
        {"symbol": "AMD", "market": "US", "priority": 1},
        {"symbol": "PDD", "market": "US", "priority": 2},
        {"symbol": "NVDA", "market": "US", "priority": 3},
        {"symbol": "AAPL", "market": "US", "priority": 4},
    ]
    assert "account" not in str(snapshot)
    assert "sources" not in str(snapshot)


def test_ai_observation_publish_retains_previous_generation_without_successful_account(
    tmp_path: Path,
) -> None:
    from src.application import multi_account_tick as mod
    from src.application.ai_decision_advice.identity import (
    ...
    publish_observation_partition(
        base=tmp_path,
        market="US",
        observed=build_observation_set(stock_holding_symbols=["NVDA"]),
        generation="us-old",
    ...
    snapshot = load_observation_set(tmp_path)
    assert snapshot is not None
    assert snapshot["partitions"]["US"]["generation"] == "us-old"
    assert snapshot["partitions"]["US"]["symbols"] == [
        {"symbol": "NVDA", "market": "US", "priority": 3}
    ]

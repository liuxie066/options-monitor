...
def _prepare_workspace(base, run_id, *, default_account):
    return prepare_tick_run_workspace(
        base=base,
        run_id=run_id,
...
def _publish_config(base, run_id, *, config, account: str = "lx"):
...
def _load_config(authority, run_id, *, base, account: str = "lx"):
    return load_account_run_config(
        authority=authority,
        base=base,
        run_id=run_id,
        account=account,
...
def test_prepare_tick_run_workspace_creates_required_dirs(tmp_path) -> None:
    workspace = _prepare_workspace(tmp_path, "20260513T010203", default_account="lx")
...
def test_prepare_tick_run_workspace_preserves_historical_runs(tmp_path) -> None:
    ...
    _prepare_workspace(tmp_path, "20260719T120000", default_account="lx")
...
def test_account_config_publication_is_run_scoped_under_overlap(tmp_path) -> None:
    ...
    def _publish(run_id: str, marker: str):
        return _publish_config(tmp_path, run_id, config=_account_config(marker=marker))
...
def test_same_run_identical_config_is_adopted_concurrently(tmp_path) -> None:
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        authorities = list(
            executor.map(
                lambda _index: _publish_config(tmp_path, "run-same", config=config),
                range(16),
            )
        )
...
def test_same_run_different_config_fails_without_overwrite(tmp_path) -> None:
    ...
    with pytest.raises(AccountRunConfigError) as raised:
        _publish_config(
            tmp_path, "run-conflict", config=_account_config(marker="replacement")
        )
...
def test_account_config_authority_rejects_cross_run_consumption(tmp_path) -> None:
    authority = _publish_config(tmp_path, "run-a", config=_account_config())

    with pytest.raises(AccountRunConfigError) as raised:
        _load_config(replace(authority, run_id="run-b"), "run-b", base=tmp_path)
...
def test_account_config_publication_rejects_unsafe_account_before_writes(
    ...
    with pytest.raises(AccountRunConfigError) as raised:
        _publish_config(
            tmp_path, "run-safe", account=account, config=_account_config(account=account)
        )
...
def test_account_config_semantic_scope_is_validated_before_writes(tmp_path) -> None:
    with pytest.raises(AccountRunConfigError) as raised:
        _publish_config(
            tmp_path, "run-semantic-mismatch", config=_account_config(account="sy")
        )
...
def test_account_config_publication_rejects_symlinked_ancestor(tmp_path) -> None:
    ...
    with pytest.raises(AccountRunConfigError) as raised:
        _publish_config(tmp_path, "run-symlink", config=_account_config())
...
def test_account_config_load_rejects_account_directory_symlink_swap(tmp_path) -> None:
    ...
    with pytest.raises(AccountRunConfigError) as raised:
        _load_config(authority, "run-swap", base=tmp_path)
...
def test_prepare_workspace_rejects_unsafe_default_account_before_writes(
    ...
    with pytest.raises(AccountRunConfigError) as raised:
        _prepare_workspace(tmp_path, "run-safe", default_account="../lx")

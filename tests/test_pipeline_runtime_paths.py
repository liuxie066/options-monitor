...
def test_scan_pipeline_uses_runtime_root_and_loaded_config_for_refresh(monkeypatch, tmp_path: Path) -> None:
    ...
    events: list[str] = []
    cfg = {"symbols": [{"symbol": "MSFT", "fetch": {"source": "opend"}, "sell_put": {"enabled": False},
                "sell_call": {"enabled": False},}],
        "portfolio": {"broker": "富途", "data_config": "portfolio.runtime.json"}, "notifications": {"enabled": False},}
    ...
    monkeypatch.setattr(multiplier_cache, "save_cache", lambda _path, cache: saved_cache.update(cache),)
    monkeypatch.setattr(multiplier_cache, "refresh_via_opend", lambda **_kwargs: events.append("refresh")
        or SimpleNamespace(ok=True, multiplier=100, source_receipt_sha256="a" * 64,),)
    monkeypatch.setattr(pipeline_watchlist, "run_watchlist_pipeline_default", _fake_run_watchlist_pipeline_default)

    rc = pipeline_runtime.main(["--config", str(config_path), "--stage", "fetch", "--no-context",
...
def test_stage_only_notification_always_builds_compact_compatibility_bundle(monkeypatch, tmp_path: Path) -> None:
    ...
    mod.run_stage_only_alert_notify(report_dir=tmp_path, stage_only="notify", want=lambda step: step == "notify",
        log=logs.append,)
...
def _set_authority_env(monkeypatch, authority) -> None:
    monkeypatch.setenv("OM_ACCOUNT_CONFIG_CANONICAL_B64", base64.b64encode(authority.canonical_bytes).decode("ascii"),)


def test_pipeline_runtime_passes_validated_account_config_payload_to_loader(monkeypatch, tmp_path: Path,) -> None:
    from src.application import pipeline_runtime as mod
    from src.application.tick_run_workspace import publish_account_run_config

    authority = publish_account_run_config(base=tmp_path, run_id="run-pipeline", account="lx", config={
            "portfolio": {"account": "lx"}, "runtime": {"marker": "validated-authority"}, "symbols": [],},)
    ...
    monkeypatch.setattr(mod.report_repo, "prepare_dirs", lambda **_kwargs: (report_dir, state_dir),)
    ...
    monkeypatch.setattr(pipeline_watchlist, "run_watchlist_pipeline_default",
        lambda **kwargs: pipeline_observed.update(kwargs) or [],)

    assert mod.main(_authority_args(authority, base=tmp_path) + ["--stage", "fetch", "--source-account-run-id",
            "run-pipeline", "--prepared-option-positions-context-manifest", str(prepared_options),
            "--prepared-option-positions-context-manifest-sha256", "b" * 64,]) == 0
    assert observed["config_payload"]["runtime"]["marker"] == ("validated-authority")
    assert observed["config_path"] == authority.state_path
    assert pipeline_observed["prepared_option_positions_context_manifest"] == prepared_options.resolve()
    assert pipeline_observed["prepared_option_positions_context_manifest_sha256"] == "b" * 64


def test_pipeline_runtime_classifies_prepared_option_context_failure(monkeypatch, tmp_path: Path,) -> None:
    from src.application import pipeline_runtime as mod
    from src.application.prepared_option_positions_context import (PreparedOptionPositionsContextError,)
    from src.application.tick_run_workspace import publish_account_run_config

    authority = publish_account_run_config(base=tmp_path, run_id="run-prepared-option-failure", account="lx", config={
            "portfolio": {"account": "lx"}, "symbols": [],},)
    prepared_options = tmp_path / "prepared-options.json"
    prepared_options.write_text("{}\n", encoding="utf-8")
    monkeypatch.setattr(mod.report_repo, "prepare_dirs", lambda **_kwargs: (tmp_path / "reports", tmp_path / "state"),)
    monkeypatch.setattr(mod, "load_runtime_pipeline_config", lambda **_kwargs: {"symbols": []},)
    from src.application import pipeline_watchlist

    monkeypatch.setattr(pipeline_watchlist, "run_watchlist_pipeline_default", lambda **_kwargs: (_ for _ in ()).throw(
            PreparedOptionPositionsContextError("payload hash mismatch")),)
    _set_authority_env(monkeypatch, authority)

    with pytest.raises(SystemExit, match=(r"\[CONFIG_ERROR\] " r"ACCOUNT_CONFIG_PREPARED_OPTION_CONTEXT_INVALID: "
            r"payload hash mismatch"),):
        mod.main(_authority_args(authority, base=tmp_path) + ["--stage", "fetch", "--source-account-run-id",
                authority.run_id, "--prepared-option-positions-context-manifest", str(prepared_options),
                "--prepared-option-positions-context-manifest-sha256", "b" * 64,])


def test_pipeline_runtime_uses_retained_generation_after_path_drift(monkeypatch, tmp_path: Path,) -> None:
    from src.application import pipeline_runtime as mod
    from src.application.tick_run_workspace import publish_account_run_config

    authority = publish_account_run_config(base=tmp_path, run_id="run-pipeline-tamper", account="lx",
        config={"portfolio": {"account": "lx"}, "symbols": []},)
    authority.state_path.write_text("{}\n", encoding="utf-8")
    observed: dict[str, object] = {}
    monkeypatch.setattr(mod.report_repo, "prepare_dirs", lambda **_kwargs: (tmp_path / "reports", tmp_path / "state"),)
    monkeypatch.setattr(mod, "load_runtime_pipeline_config",
        lambda **kwargs: observed.setdefault("config", kwargs["config_payload"]) or {"symbols": []},)
    from src.application import pipeline_watchlist

    monkeypatch.setattr(pipeline_watchlist, "run_watchlist_pipeline_default", lambda **_kwargs: [],)
    _set_authority_env(monkeypatch, authority)
...
def test_pipeline_subprocess_command_forwards_complete_config_authority(monkeypatch, tmp_path: Path,) -> None:
...
def test_pipeline_subprocess_command_forwards_experience_contract(monkeypatch, tmp_path: Path,) -> None:
    from src.infrastructure import external_services as mod

    observed: dict[str, object] = {}
    monkeypatch.setattr(mod, "run_command", lambda command, **kwargs: observed.update(
            {"command": list(command), **kwargs}) or object(),)

    mod.run_pipeline_script(vpy=tmp_path / "python", base=tmp_path, config=tmp_path / "config.override.json",
        report_dir=tmp_path / "reports", state_dir=tmp_path / "state", experience=True, account_display_name="美股模拟期权账户",
    )

    command = observed["command"]
    assert command[command.index("--mode") + 1] == "scheduled"
    assert command[command.index("--account-display-name") + 1] == ("美股模拟期权账户")
    assert "--experience" in command


@pytest.mark.parametrize("extra", [["--mode", "dev", "--stage", "fetch"], ["--mode", "dev", "--stage-only", "notify"],],
)
def test_experience_pipeline_rejects_incomplete_stage(tmp_path: Path, extra: list[str],) -> None:
    from src.application import pipeline_runtime

    with pytest.raises(SystemExit, match="manual full scan"):
        pipeline_runtime.main(["--config", str(tmp_path / "unused.json"), "--report-dir", str(tmp_path / "report"),
                "--state-dir", str(tmp_path / "state"), "--experience", "--account-display-name", "美股模拟期权账户", *extra,])

...
def _render_summary(**fields: Any) -> str:
    return _render_via_alert_engine(_summary_row(**fields))
...
## 高优先级
- [英伟达](NVDA) | sell_call | 2026-06-18 180C | 年化 12.30% | 净收入 240.40 | DTE 44 | Strike 180 | 保守 | ccy USD | ask 2.500 | bid 2.300 | mid 2.400 | delta 0.16 | cover 2 | shares 200(-0) | 已通过准入，可作为 CC 备选。
"""
...
"""
    ...
    assert "**[sy] 英伟达｜CC｜2026-06-18 180C**" in out
...
## 高优先级
- NVDA | sell_call | 2026-06-18 180C | 年化 - | 净收入 240.40 | DTE 44 | Strike nan | 保守 | ccy USD | ask 2.500 | bid 2.300 | mid 2.400 | delta nan | cover nan | shares nan | 已通过准入，可作为 CC 备选。
"""
    out = _render_legacy("", alerts, account_label="SY")
...
def test_notify_symbols_markdown_put_chain_uses_upstream_fields_when_available() -> None:
    out = _render_summary(
        symbol="0700.HK", top_contract="2026-04-29 460P", annualized_return=0.1721,
        net_income=557.00, dte=26, strike=460.0, delta=-0.23, iv=0.41,
        cash_required_cny=110720.0, mid=5.72, bid=5.58, ask=5.86, option_ccy="HKD",
    )
...
def test_notify_symbols_markdown_put_chain_shows_event_risk() -> None:
    ...
    summary_row = normalize_processor_row(
        _summary_row(
            symbol="AAPL", top_contract="2026-06-19 180P", annualized_return=0.18,
            net_income=210.0, dte=24, strike=180.0, delta=-0.22, iv=0.38,
            cash_required_usd=18000.0, mid=2.1, option_ccy="USD",
            event_flag=True, event_types="earnings", event_dates="2026-06-10",
        )
    )
...
def test_notify_symbols_markdown_put_chain_shows_same_symbol_usage_from_summary_fields() -> None:
    out = _render_summary(
        symbol="3690.HK", top_contract="2026-05-28 75P", annualized_return=0.128,
        net_income=468.0, dte=36, strike=75.0, risk_label="保守", delta=-0.16, iv=0.4138,
        cash_required_cny=32715.0, cash_secured_used_cny_total=200000.0,
        cash_secured_used_cny_symbol=45000.0, mid=0.965, option_ccy="HKD",
    )

    assert "保证金占用=¥32,715 (CNY)" in out
    assert "同标的CSP占用=¥45,000" in out
...
def test_notify_symbols_markdown_put_falls_back_to_usd_margin_when_cny_margin_missing() -> None:
    out = _render_summary(
        symbol="0700.HK", top_contract="2026-04-29 460P", annualized_return=0.1721,
        net_income=557.00, dte=26, strike=460.0, delta=-0.23, cash_required_usd=58880.0,
        cash_free_cny=200000.0, mid=5.72, option_ccy="HKD",
    )
...
def test_notify_symbols_markdown_put_chain_missing_fields_keep_reasons() -> None:
    out = _render_summary(
        symbol="NVDA", top_contract="2026-06-18 156P", annualized_return=0.1,
        net_income=524.99, dte=76, strike=156.0,
    )
...
def test_notify_symbols_markdown_put_chain_shows_linked_call_hint() -> None:
    out = _render_summary(
        symbol="NVDA", top_contract="2026-06-19 95P", annualized_return=0.273,
        net_income=307.65, dte=44, strike=95.0, delta=-0.25, iv=0.42,
        cash_required_usd=9500.0, mid=3.1, bid=3.0, ask=3.2, option_ccy="USD",
        linked_call_contract="2026-06-19 110C", linked_call_count=2, linked_call_ask=1.5,
        linked_call_delta=0.32, linked_call_net_credit=145.33, linked_call_scenario_score=0.0458,
    )
    ...
    assert "全账户CSP占用" not in out


def test_notify_symbols_markdown_combo_yield_layout() -> None:
    out = _render_summary(
        symbol="NVDA", strategy="yield_enhancement", top_contract="2026-06-19 95P+110C",
        annualized_return=1.0142, net_income=145.33, dte=44, strike=95.0, option_ccy="USD",
        mid=1.453, put_bid=3.0, put_strike=95.0, call_strike=110.0, call_ask=1.5,
        call_delta=0.32, call_candidate_count=2, net_credit=145.33, scenario_score=0.0458,
        expected_move=14.24, expected_move_iv=0.41, combo_spread_ratio=0.18,
    )
...
def test_combo_yield_notification_ignores_option_contract_display_name(tmp_path) -> None:
    ...
    row = normalize_processor_row(
        _summary_row(
            symbol="PDD", strategy="yield_enhancement", top_contract="2026-06-26 78P+88C",
            annualized_return=0.41, net_income=128.34, dte=15, strike=78.0, risk_label="激进",
            option_ccy="USD", mid=1.283, put_bid=1.68, put_strike=78.0, call_strike=88.0,
            call_ask=0.35, call_delta=0.12, call_candidate_count=1, net_credit=128.34,
            scenario_score=1.78, expected_move=6.27, expected_move_iv=0.3892,
        )
    )
...
def test_alert_engine_high_priority_orders_by_strategy_then_strength() -> None:
    ...
    rows = [
        ...
        _summary_row(
            ...
            annualized_return=0.11, net_income=110.0, dte=30, strike=430.0, risk_label="保守",
            cover_avail=1, shares_total=100, shares_locked=0, if_exercised_total_return=0.16,
        ),
        _summary_row(
            symbol="BABA", strategy="yield_enhancement", top_contract="2026-06-19 80P+95C",
            annualized_return=0.30, net_income=90.0, dte=30, strike=80.0, risk_label="保守",
        ),
    ]
...
def test_alert_engine_missing_numeric_fields_do_not_abort_alert_build() -> None:
    ...
    rows = [
        _summary_row(
            symbol="NVDA", top_contract="2026-06-19 100P", annualized_return=pd.NA,
            net_income=pd.NA, dte=pd.NA, strike=pd.NA, risk_label=pd.NA,
        ),
        _summary_row(
            symbol="AAPL", top_contract="2026-06-19 180P", annualized_return="",
            net_income="", dte="", strike="", risk_label="",
        ),
    ]
...
def test_build_notification_keeps_per_strategy_capacity() -> None:
    ...
    call_line = (
        ...
        "DTE 30 | Strike 180 | 保守 | ccy USD | mid 1.000 | cover 1 | shares 100(-0) | 已通过准入，可作为 CC 备选。"
    )
...
def test_build_notification_keeps_medium_strategy_when_high_exists() -> None:
    ...
    medium_call = (
        ...
        "DTE 30 | Strike 430 | 保守 | ccy USD | mid 0.800 | cover 1 | shares 100(-0) | 已通过准入，可作为 CC 备选。"
    )
    alerts = (
        "# Symbols Alerts\n\n"
        "## 高优先级\n"

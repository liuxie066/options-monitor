...
- 历史验证覆盖持仓 SQLite、Feishu 镜像同步、agent plugin smoke、符号策略清理、auto-sync 工作流相关测试：159 passed。旧界面专项用例后续已退役，不再作为当前验证命令。

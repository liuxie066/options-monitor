...
CASES = (
    EvalCase(
        ...
        required_response_terms=("mtd", "账户", "期权净现金流", "卖出期权胜率", "买入期权胜率", "期权收益率", "年化收益率"),
        ordered_response_terms=("期权净现金流", "卖出期权胜率", "买入期权胜率", "期权收益率"),
    ...
    EvalCase(
        ...
        forbidden_primary_input_fields=("account",),
        required_response_terms=("ytd", "账户", "期权净现金流", "卖出期权胜率", "买入期权胜率", "期权收益率", "年化收益率"),
        ordered_response_terms=("期权净现金流", "卖出期权胜率", "买入期权胜率", "期权收益率"),
        forbidden_response_terms=("source_event_id", "evidence_fact_id", "cash_conversion:", "CNY 折算为空"),
        max_response_chars=1200,
    ),
...
def apply_human_reviews(
    ...
    if payload.get("schema_version") != "om.bot.p1_eval.v4":
        raise SystemExit("review report schema_version must be om.bot.p1_eval.v4")

...
### Bug Fixes
- 修复成交接收心跳看门狗把健康账号长期判为失联并实际投递告警的问题：判据只看单元活性与心跳新鲜度（1200 秒，约实测单轮最坏值的 2 倍），阶段标签只作为告警上下文；OpenD 掉线进入重连期间仍判失联，不会假报恢复。同时修复状态文件长期停在 `starting`、`runtime_status` 的 `listener_status` 恒为 `partial` 的问题。

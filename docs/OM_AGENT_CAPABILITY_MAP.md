...
| Health/readiness | `./om-agent run --tool healthcheck`, `/health`, `/doctor` | `healthcheck` | Runtime config, data config, OpenD readiness probe, service/channel diagnostics | None | Core Read | Decide when readiness beats runtime artifact reads | `./om-agent run --tool healthcheck --input-json '{"config_key":"us"}'` | `om-agent`, Inbound |

...
> Historical design note. This document is retained for analysis-workspace
> rationale, but it is not the current architecture terminology authority.
...
`analysis_query` 打造成 Inbound Assistant 可使用的受控只读分析工作区。

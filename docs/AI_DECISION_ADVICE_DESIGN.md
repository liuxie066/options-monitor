...
- 当前标的和币种集中度（相对权重）；
- 新增一张的方向（Sell Put 指派增加持股 / Covered Call 被叫走减少持股）、
  合约股数与指派名义金额；
- 同标的已有 Put/Call 的方向叠加数量；
- 到期日集中关系；
- 与现有组合结构的重叠关系。

组合输入是相对权重，无法确定性地推导新增一张后的绝对权重；投影只输出可
确定计算的事实，不伪造 after-trade 权重数值。这些值是模型可引用的事实，
不由模型自行计算。v1 不建设组合级 Greeks 聚合。

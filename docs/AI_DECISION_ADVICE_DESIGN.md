...
- Advice 完成时仍在对应策略内展示实质结论、原因和必要来源；
- Advice 整体未完成且至少一个策略有原始候选时，只在候选模块前显示一次
  `AI｜未完成；以下仅为策略排序，未经综合判断。`；无原始候选的策略模块仍明确显示
  `本轮无可展示的策略排序。`，不能借用其他策略的候选状态；
- 合法零候选在对应策略下压缩为一句，不再叠加 `AI建议` 和
  `策略候选` 空标题；
- 候选仍保留真实策略排序及正式指标，只省略可由策略标题直接推知的
  `策略候选` 子标题；
- 通用 `partial_data`、单个合约无当前报价、无可用到期日等合约级市场结果不进入
  用户侧“提醒”；但 sealed 候选快照已经给出明确硬证据原因时必须具体提示，例如
  `term_matched_rv_unavailable` 显示为期限匹配 RV 证据不可用；所有原始事实仍保留在结构化简报和审计中；
- 预取器的 `fetched` 与 `ok` / `cached` 同属成功状态，不得投影为数据缺口；
- “提醒”只展示已有明确来源证据的行情预取失败、策略扫描失败、候选快照不可用/校验失败，
  或 sealed 快照已识别的明确硬证据缺失；通用不完整状态不得被渲染为获取错误；
- 卡片未展示逐候选手数容量时，不显示“手数不能相加”的无上下文提醒。

...
- Advice 整体未完成时，只在候选模块前显示一次
  `AI｜未完成；以下仅为策略排序，未经综合判断。`；
- 合法零候选在对应策略下压缩为一句，不再叠加 `AI建议` 和
  `策略候选` 空标题；
- 候选仍保留真实策略排序及正式指标，只省略可由策略标题直接推知的
  `策略候选` 子标题；
- `partial_data`、单个合约无当前报价、无可用到期日等合约级市场结果不进入
  用户侧“提醒”；它们仍保留在结构化简报和审计中；
- 预取器的 `fetched` 与 `ok` / `cached` 同属成功状态，不得投影为数据缺口；
- “提醒”只展示已有明确来源证据的行情预取失败、策略扫描失败或候选快照
  不可用/校验失败；通用不完整状态不得被渲染为获取错误；

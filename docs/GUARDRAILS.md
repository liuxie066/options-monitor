...
Goal: reduce the pull-request regression wall clock without selecting fewer tests or weakening the single required `guardrails` result. Success requires the complete automatically discovered suite to pass with four workers and preserve the baseline outcome counts: 7057 passed, 1 skipped, with no deselected or newly xfailed tests. On the same `ubuntu-latest` pull-request workflow, the regression step must complete within 199 seconds and the whole `guardrails` job within 252 seconds, at least 10% below the two-worker 220.87-second step and 280-second job baselines recorded on 2026-09-14. A result within 10% of either limit must be rerun once because hosted-runner timing is noisy. The serial command remains available for diagnosis.

The workflow uses pinned `pytest-xdist` and `pytest -n 4 --dist loadfile --max-worker-restart=0`. A fixed worker count matches the public Linux runner's four CPUs and keeps CI resource use predictable; `loadfile` keeps each test module in one process, while normal pytest collection still discovers every test. Zero worker restarts makes crashes visible. Lint, privacy checks, smoke tests, release resolution, job name, and release trigger behavior remain unchanged.
...
Worker-count changes must update the full-regression invocation and its existing workflow contract test, which enforces the exact command, dependency declaration, exact pin, and absence of `-k`, `--ignore`, `--deselect`, or test-path selection. Run that contract test, compare serial collection with the complete parallel result, then use a real pull-request run for the timing acceptance above. The change affects development CI only; it does not alter runtime code, production data, notifications, releases, or deployment.

...
这套“只修不增”边界应采用，但有三点校正：

1. Strategy Lab 没有独立的 `./om strategy-lab`；公开入口是
   `./om research strategy-lab ...`。
2. 通用 Strategy Lab `readiness / experiment / proposal / llm-context` 已退役。当前只保留 formal
   `top1-loop` 和 recorder maintenance `update`；新的探索性 replay 分析进入 Shadow Replay，新的
   formal Top1 行为进入 `src/application/strategy_lab/top1/`。
3. 退出条件只是开始删除审计的门槛，不是删除授权。删除前仍需证明无运行时 caller、无公开合同依赖、
   兼容数据已退出，并运行 owning facade 测试。
...
- `./om research shadow-replay ...`、`./om research strategy-lab top1-loop ...`；
- recorder 所需的 `./om research strategy-lab update ...`，仅维护 dataset / mark / outcome。

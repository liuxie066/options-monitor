...
- **状态**：Phase 2 本地实现完成；尚未通过远端自然 Tick 交付门槛
- **更新时间**：2026-08-31
...
本文只描述当前源码。研究链存在不代表已跑出 20 日结论；真实验收必须以具体实验的
Research Receipt 为准。10 日隐藏验证尚未实现。
...
`confirm-research` 只接受当次重建后仍可用且 hash 相同的 preview；`research execute` 需要
操作员重复显式调用，每次最多执行一个 provider 逻辑证据单元。`status` 和 `receipt` 只读，只依赖
...
- 10 日隐藏验证、advance timer、outcome 和 Final Receipt；
- MCP、Skill、飞书、并行实验、自动采用与生产配置写入。

因此，没有具体 Research Receipt 时不能宣称已有 research leader；当前也不存在
hidden-validation 进度或 Final Receipt。
...
- Phase 1 远端自然 Tick 证据仍是合并或交付门槛，本地测试不能替代。

...
| Planner context budget | capability selection 使用当前问题/短 follow-up/evidence gap 选择必要 view，不恢复全量 manifest |
...
| `planner_manifest_budget` | 验证 analysis views 被按任务裁剪、selection source 正确、manifest 未膨胀回全量 |
...
| Golden eval | M4 本地验收通过 | `assistant_agent_eval` 已有 45 条 fixture，覆盖收益对比、收益对比缺 sy 后同 scope follow-up、收益来源 summary-only 后 breakdown follow-up、指派正股、指派正股 fresh quote、指派正股缺 quote 后 refresh follow-up、stale quote freshness、symbol identity resolve、candidate_filter_explain observed rejection、candidate_filter_explain missing trace、candidate partial-confidence root-cause、runtime why、runtime conflict/stale、runtime scheduler market-window skip、runtime notification missing、runtime notification conflict、upgrade receipt missing version、upgrade follow-up operation_timeline、upgrade conflict / command log missing、release tag not enough、release no matching rows、release published/failed、release/outcome conflict、old operation timeline、scope expansion、SQL-only scope expansion、prompt injection from tool output、write preview no apply，以及 planner context budget / selection source / recent read hint 断言；harness 已支持 final response、answer guard、coverage status/gap、diagnostic domain/status、action safety、preview no apply、follow-up tool call、planner context budget 断言；测试会解析 6.6.2 P2 minimum case checklist，并要求 agent eval 与 trace route fixture mapping 完整对齐 | 后续只追加线上回归样本，不为单一问题写专用模式 |
...
- `assistant_agent_eval` 已扩展为 45 条 fixture，新增覆盖升级状态问题触发一次只读
  `operation_timeline` follow-up 后合并证据生成答案的路径，并覆盖 planner context budget：
  短 follow-up 使用 recent read hint、明确当前问题覆盖旧上下文、evidence gap suggested views 驱动
  scoped manifest。
...
- eval harness 支持普通回答、action safety、planner preview、planner context 四类 fixture，但这些只是测试入口，
  不是用户可见模式。
...
| Gate | 命令 | 通过证据 | 失败回退 |
|---|---|---|---|
| fixture 格式 | `jq -c . tests/fixtures/assistant_agent_eval.jsonl` | 45 条 JSONL 都能逐行解析 | 停止发布，先修 fixture；不要删除失败样本 |
...
| agent eval | `python3 -m pytest tests/test_assistant_agent_eval.py` | stale/conflict/release_tag/scope/planner-context 等 fixture 全部通过，P2 agent-eval gap group、6.6.2 minimum case mapping、6.20 online sample contract、planner context budget 和集中 UX leak guard 通过 | 回到 evidence / coverage / verifier 修根因，不加固定文案 |

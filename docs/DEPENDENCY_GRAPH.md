...
- Python files scanned: 677 (`src`: 394, `domain`: 52, `scripts`: 7, `tests`: 224)
- Internal import edges: 3603 total, 1684 production/script edges excluding tests
- Parse errors: 0
...
flowchart LR
  ...
  scripts -->|2| storage
  storage -->|1| domain
  tests -->|1317| application
  tests -->|289| domain
  tests -->|2| domain_services
  tests -->|95| infrastructure
  tests -->|172| interfaces
  tests -->|8| scripts
  tests -->|18| storage
```
...
| from | to | imports |
|---|---|---|
| tests | application | 1317 |
| tests | domain | 289 |
| tests | interfaces | 172 |
...
| src.application.positions | src.application | 13 |
...
| src.application.positions | domain.domain | 5 |
...
| src.application.assistant.agent_loop | 23 |
...
| src.application.channels.wechat_clawbot.inbound | 19 |

...
平仓、指派、行权和到期关闭首次写入时，显式时间必须为正整数毫秒，并保留其历史
发生时间；后续入账或 void 事件不会将它推到当前日期。多 lot 生命周期操作沿用后续
事件逐笔加 1 毫秒的排序约定。未提供时间时才使用当前时间与账本最新时间后的单调值。
历史补录仍须通过完整候选投影校验；早于开仓或与当时目标身份冲突的关闭会被拒绝。
同一请求标识重试返回原事件，重新传入时间不会修改既有事实。
...
候选读取退化时，仓储发出 WARN，包含事件表、`missing` 或
`definition_mismatch` 原因和当前行数；只读 `projection-migration status` 的
...
估值和 FX 证据共用 `domain/domain/performance/models.py` 的
`_validate_evidence_kind`，不改变收益报表的产品范围。

### 目标与范围

消除随批量条数增长的重复全图校验，同时保持事实身份、来源身份、纠正引用和持久化结果。
范围仅含共享校验函数与必要回归测试；不调整 SQL、schema、连接、锁、读取范围、事务、
缓存或消费者，不重写纠正图遍历算法，不优化其它数据库方法。

当前调用链为 `import_envelope -> parse_evidence_envelope -> validate_evidence_facts`，
然后预检读取与校验；apply 路径在 `BEGIN IMMEDIATE` 后再次读取和校验，再插入并提交。
传入类型化 `EvidenceEnvelope` 时跳过解析，但仍经过预检与事务内校验。
`read_all` 也使用同一个校验入口，既有事实必须独立成立，不能靠 incoming 修复坏历史。

每种证据先校验 existing 图，再逐条检查 incoming，最后校验合并图。
无纠正链时，N 条 existing 和 m 条 incoming 的图校验成本为 O(N + m)。

### 校验顺序与失败语义

保留 existing 身份建图与首次完整校验。incoming 仍按原顺序逐条检查 fact_id、
source identity 的 payload 冲突、自我纠正、目标必须存在于 existing 或更早 incoming、
纠正前后 exact identity 一致，然后加入现有字典。仅将循环内的全图校验移到循环后；
incoming 非空时对合并图再校验一次，空批次不重复校验 existing。

两个事实类型分别校验，不能跨类型补齐引用。相同 payload 的重试仍合法，身份相同但
payload 不同仍拒绝。异常类别、诊断文本和多项非法输入的校验执行顺序保持不变。
既有完整校验与逐条前向限制保证加入合法事实时不会凭空形成环。最终校验检查图中的环、
缺失目标和 identity 不匹配；fact/source payload 冲突仍由逐条检查负责。

仓库的预检、事务内重读与重验、schema gate、插入计数、幂等计数、commit/rollback 和
readback 行为保持原样。dry-run 不建表、不写事实；失败不得留下部分事实或迁移。
不新增状态、配置或依赖。移除仓库预检或事务重验会改变一致性边界，因此不采用；
新增增量图结构没有必要，也不采用。

### 验证边界

- 经公开解析/校验与仓库入口验证两类事实、合法纠正链、existing 中非拓扑顺序的纠正、
  同批较早引用、向后引用/缺失引用、自引用、identity 不匹配、fact/source 冲突和坏历史环。
  补齐现有测试未覆盖的拒绝场景，包括跨类型引用，并断言原 ValueError 诊断；多个非法
  条目仍按原顺序报告首个错误。类型化 envelope 的非法输入也必须经过仓库入口验证。
- 保留 dry-run、幂等重试和持久化读回断言。用合法类型化 envelope，在临时数据库中让
  第一条真实插入后第二条强制失败，重新打开数据库确认没有部分事实或 schema 迁移；
  解析期拒绝不能代替事务回滚证据。复用 `tests/test_performance_evidence_sqlite.py`
  和 `tests/test_private_storage.py`，不新增测试框架。
- 确定性性能回归：对两种批量大小使用同一公开入口，计量 `_validate_correction_graph`
  调用次数，要求不随 incoming 条数增长；同时核实处理结果，不能靠跳过校验通过。
  全图函数仍会执行，不要求它从 profile 消失，不把机器耗时阈值写进 pytest。
- 耗时基准：复用临时合成数据库基准，分别测 1,000、3,000、10,000 条无纠正 FX，
  每档三次取中位数并记录 cProfile，核对插入数、读回和重试结果。前后使用同一解释器、
  已核实基线及仅含本项改动的版本、数据生成器与存储位置；保留前后原始数据，要求全图
  调用次数不再逐条增长且大批量导入中位耗时下降。噪声影响时增加配对样本，不能猜测通过。

单次完整图遍历对长纠正链仍可能为平方成本；本优化只消除批次循环的额外乘数。
深链算法与生产数据分布不由合成批量样本证明，owner 仍为共享校验函数。
前后测量必须绑定实际源码版本，不能把旧测量当成新版本的验收证据。
...
同账户、券商、标的、币种的出售数量恰好等于全部匹配批次的剩余库存时，可在一个
`assigned_stock_events` 事件内保存 `sale_allocations`。每个分配包含独立股票批次、数量、
费用及现金换算，数量和费用必须合计等于原成交，原券商执行身份只持久化一次。
所有批次一起验证和提交；部分出售仍须唯一匹配，不能自行选择 FIFO 或猜测批次。

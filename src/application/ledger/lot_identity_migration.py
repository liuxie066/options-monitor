...
2. **D1/D2's rebuild is gated on the column contract, not on a flag.** The
   column contract is exact-set equality (``repository_projection_schema.py``),
   so a rebuilt ``position_lots`` without ``expiration``/``record_id`` is
   ``column_contract_open`` → heads ``untrusted`` → tail publish
   ``RuntimeError`` for as long as the running code still declares those
   columns (§13.5 R7). ``apply`` therefore reads
   ``POSITION_LOTS_COLUMN_CLASSIFICATION`` — the contract that is actually in
   force — and defers the destructive half exactly while it still declares
   either retired column, with the reason string it has always reported. The
   release that tightens the contract (§9.5 M6 step 5) is the same release that
   enables the rebuild: there is no second switch to flip and no "forgot to
   flip it" failure mode. §9.5 M2 also fixes the granularity — D1's
...
3. **D3's rewrite is gated on the same fact, and cannot drop a fact.**
   ...
   兼容读窗口，否则存量行读不出" — and while the contract still declares the
   retired columns the read side is by definition still in that window. When
   the gate opens, the rewrite aligns each row to ``PositionLot.to_dict()``'s
   key set and materializes every carried key at the target
   ``CARRIED_DROPPED_KEYS`` declares for it; a key ``_drop_disposition`` calls
   ``lost`` aborts the whole ``apply`` instead of disappearing. The classifier
   is therefore the rewrite's gate, not a parallel vocabulary beside it.
...
def _contract_still_declares_retired_columns() -> tuple[str, ...]:
    """Which retired columns the running code's column contract still declares.

    The gate is *derived from the contract in force*, never from a flag: the
    column contract is exact-set equality and is also the publish path's hard
    door, so a store rebuilt while the contract still declares
    ``expiration``/``record_id`` is ``column_contract_open`` → head
    ``untrusted`` → tail publish ``RuntimeError`` (§13.5 R7). Reading the
    contract answers "may the rebuilt shape exist at all?" exactly, where a
    flag would answer only "did someone remember to set it?".

    Returns the still-declared names, in ``RETIRED_LOT_COLUMNS`` order; empty
    means the contract has tightened and the destructive half may run.
    """

    declared = set(POSITION_LOTS_COLUMN_CLASSIFICATION)
    return tuple(name for name in RETIRED_LOT_COLUMNS if name in declared)
...
def _aligned_lot_payload(
    ...
    for key, value in fields.items():
        if key in target_keys or not _non_empty(value):
            continue
        disposition, reason = _drop_disposition(key, value, fields, surviving_columns)
        if disposition == "lost":
            raise RuntimeError(
...
def _rewrite_lot_payloads(
    ...
    *,
    ...
    ``apply`` runs the alignment only once the contract gate says the read side
    ...
    for row in rows:
        ...
        if align_to_lot_shape:
            aligned = _aligned_lot_payload(fields, row)
    ...
    *,
    ...
    with _write_connection(path) as conn:
        ...
        try:
            ...
            # The gate is read inside the transaction, from the contract in
            # force, so the store and the code it is being migrated by cannot
            # disagree about which shape is allowed to exist.
            deferred_by = _contract_still_declares_retired_columns()
    ...
    # shape across the window. Only the status changes, and while the contract
    # still declares the retired columns the reason is the one this batch has
    # always reported.
    if deferred_by:
        ...
        # Only in this state: the default path's report is a contract this batch
        # must not move, and a key nobody reads while the steps are deferred is
        # noise in every pre-window receipt.

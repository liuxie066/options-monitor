...
def render_one(row) -> str:
    ...
    body = [
        ...
        "",
        f"DTE: {int(row['dte'])}",
        f"卖出参考价(mid): {num(row['mid'])}  (净权利金年化按 mid / spot 口径计算)",
        "",
        ...
        f"IV/RV: {num(row.get('iv_rv_ratio'))} | IV-RV: {pct(row.get('iv_minus_rv'))} | abs(delta): {num(row.get('abs_delta'))}",
        f"短波动: {row.get('short_gamma_profile', '-')} / {row.get('short_vega_profile', '-')}",
        f"右尾机会成本: {num(row.get('call_gap_up_opportunity_cost_cny'))} CNY | 占NAV: {pct(row.get('call_gap_up_opportunity_cost_nav_pct'))} | /premium: {num(row.get('call_gap_up_opportunity_cost_to_premium'))}",
        "",

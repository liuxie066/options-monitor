...
from src.application.bot.contracts import AppResult, BotRequest, ExecutionContract, new_id
from src.application.bot.scene import GENERAL_SCENE
...
def prepare_contract(
    ...
    *,
    ...
    now = time.monotonic()
    received = request.received_monotonic if request.received_monotonic is not None else now
    deadline = min(received + 180.0, request.deadline_monotonic if request.deadline_monotonic is not None else received + 180.0)
    if not math.isfinite(received) or not math.isfinite(deadline) or received > now:
        return AppResult(status="not_ready", ok=False, error={"code": "CONFIG_ERROR"}, user_response="请求计时信息无效。")
    if deadline <= now:
        return AppResult(status="failed", ok=False, error={"code": "BUDGET_EXHAUSTED", "reason": "prepare"}, user_response="请求在准备阶段已到达本次预算，请继续以开始新一轮。", request_id=request.request_id)

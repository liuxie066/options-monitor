...
def parse_deterministic_text(text: str, *, now_fn: Callable[[], date] | None = None) -> PerceptionResult:
    ...
    Free-form routing is being rebuilt. This compatibility entry only accepts
    explicit slash commands so old imports cannot silently re-enable keyword-
    based business routing.
    """
    ...
    if not raw:
        raise AgentToolError(
            ...
            hint="自由问答正在重建中；显式控制请使用 slash command。",
        )

...
def handle_assistant_turn(
    ...
    *,
    ...
    response = _handle_assistant_message_response(
        request,
    ...
    return _assistant_turn_result_from_legacy_response(response)
...
def handle_assistant_message(
    ...
    *,
    ...
    return dict(turn.legacy_response or {})


def _handle_assistant_message_response(
    request: AssistantRequest,
    *,
    audit_store: InboundAuditStore | None = None,
    execute_tool_fn: ExecuteToolFn = execute_tool,
    ...
    if not runtime_settings.enabled:
        ...
        response = _with_assistant_turn_result(response, route="disabled")
    ...
    response = _with_assistant_turn_result(response, route=perception_engine.route)
...
def _with_assistant_turn_result(response: dict[str, Any], *, route: str) -> dict[str, Any]:
    ...
    turn_result = _assistant_turn_result_from_response(response, route=route, assistant_meta=assistant)
    assistant["turn_result"] = turn_result.public_payload()
    meta["assistant"] = assistant
    return {**response, "meta": meta}


def _assistant_turn_result_from_legacy_response(response: dict[str, Any]) -> AssistantTurnResult:
    meta = response.get("meta") if isinstance(response.get("meta"), dict) else {}
    assistant = meta.get("assistant") if isinstance(meta.get("assistant"), dict) else {}
    route = str(assistant.get("route") or "")
    return _assistant_turn_result_from_response(
        response,
        route=route,
...
def _assistant_turn_result_from_response(
    response: dict[str, Any],
    *,
    route: str,
    ...
    return AssistantTurnResult(
        ...
        legacy_response=dict(legacy_response) if isinstance(legacy_response, dict) else None,
    )

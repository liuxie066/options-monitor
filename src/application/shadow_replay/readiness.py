...
def summarize_shadow_replay_readiness(
    *,
    candidate_snapshots: list[dict[str, Any]] | tuple[dict[str, Any], ...],
    filter_decisions: list[dict[str, Any]] | tuple[dict[str, Any], ...],
    base: Path,
    ...
    candidate_evidence_coverage: dict[str, Any] | None = None,
    min_sample: int = 30,
) -> dict[str, Any]:
    ...
    resolved_sources = unique(resolve_many(source_paths, base=root))

    candidate_rows = dedupe_snapshots([dict(row) for row in candidate_snapshots])
    decision_rows = [dict(row) for row in filter_decisions]
    ...
    apply_candidate_evidence_gate(analysis, candidate_evidence_coverage)
    result = {
        "schema_version": READINESS_SCHEMA_VERSION,
        "source": {
            "candidate_evidence_coverage": candidate_evidence_coverage,
            "candidate_snapshot_count": len(candidate_rows),
            "filter_decision_count": len(decision_rows),
            "snapshot_source_paths": [
                safe_rel(path, base=root) for path in resolved_sources
            ],
    ...
    close_paths = _close_facet_paths(
        resolved_sources + resolved_traces + resolved_marks + resolved_outcomes
    )
    if close_paths:

...
def _symbol_config_read_tool(
    ...
    if not raw_symbol:
        raise AgentToolError(
            ...
            hint="例如：泡泡玛特 CSP max strike 是多少。",
        )
...
SYMBOL_CONFIG_READ_TOOL = build_agent_tool(
    ...
    input_schema={
        ...
        "strategy": "optional csp|cc|sell_put|sell_call|covered_call|combo_yield",
    ...
    safe_default_input={},
    examples=(
        {"input": {"config_key": "hk", "symbol": "泡泡玛特", "strategy": "csp", "field": "max_strike"}},
...
MANAGE_SYMBOLS_TOOL = build_agent_tool(
    ...
    input_schema={
        ...
        "sell_put_enabled": {"type": "boolean", "description": "add-only CSP enable switch"},
        ...
        "sell_call_enabled": {"type": "boolean", "description": "add-only CC enable switch"},

...
OPTION_PERFORMANCE_REPORT_TOOL = build_agent_tool(
    ...
    input_schema={
        ...
        "period": {
            ...
            "description": "Choose one period: mtd/ytd use an optional as_of_date; month requires month; year requires year.",
        },
        "as_of_date": {
            "type": ["string", "null"],
            "format": "date",
            "description": "Optional cutoff only when period is mtd or ytd.",
        },
        "month": {
            "type": ["string", "null"],
            "pattern": "^\\d{4}-(0[1-9]|1[0-2])$",
            "description": "Required YYYY-MM only when period is month.",
        },
        "year": {
    ...
    copilot_input_schema={
        ...
        "properties": {
            ...
            "as_of_date": {
                "type": "string",
                "format": "date",
                "description": "Optional cutoff only when period is mtd or ytd and the current message explicitly supplies it.",
            },
            "month": {
                "type": "string",
                "pattern": "^\\d{4}-(0[1-9]|1[0-2])$",

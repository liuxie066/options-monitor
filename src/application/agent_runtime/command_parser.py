...
def parse_agent_command(text: str, *, now_fn: Callable[[], date] | None = None) -> InboundIntent | None:
...
def _intent(name: str, arguments: dict[str, object] | None = None) -> InboundIntent:
    return InboundIntent(name=name, arguments=dict(arguments or {}), parser="command", confidence=1.0)
...
def _parse_positions(command: str, args: list[str]) -> InboundIntent:
...
def _parse_income(command: str, args: list[str], *, today: date) -> InboundIntent:
    account: str | None = None
    month: str | None = None
    for arg in args:
        normalized = arg.lower()
...
def _parse_runs(command: str, args: list[str]) -> InboundIntent:
    if not args:
...
def _parse_logs(command: str, args: list[str]) -> InboundIntent:
    if len(args) != 1:
